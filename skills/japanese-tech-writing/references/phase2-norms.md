# Phase 2 執筆規範 (japanese-tech-writing)

[[japanese-tech-writing]] Phase 2 の詳細規範｡ドラフトを書き始める前に本ファイルを Read する｡
節番号 (2.1〜2.17) は [[proofread]] / [[textlint-check]] から参照されるため変更しない｡

---

# Phase 2: 執筆 (ドラフトを書きながら)

Phase 1 の構成案 (見出し草案 + scope 節) を埋めていく｡執筆中は以下の規範を意識する｡
...
- 中点 (・) は全角を使い、日本語の並列に使ってよい｡半角の `･` に置き換えない (2.7 と整合)｡
...
パラグラフライティングを基本とする｡段落は論証の一歩であり、読者は段落単位で論理を追えるようにする｡
...
文章の論理にツッコミどころを残さない｡書き上げたら、読み手の反論を先回りして次の点を点検する｡(Phase 3 の self-check 項目 4・5 もこの節を根拠にする)
...
- **能動態を既定とする** (fofr 由来で強化)｡受動態は事実の所在が問題になる場合 (例: 「障害が報告された」、行為者を伏せたい場合) のみ｡英語の "be done" 直訳の受動態を慎む｡
- **fofr の "address reader as you" は採用しない**｡日本語では読者を「あなた」と呼ばず、役割名 (「開発者」「読者」「設計者」など) で書く｡二人称の呼びかけは、場面への導入 (「〜としよう」) や章・本の結びなど、限られた要所にとどめる｡
...
典拠: k16shikano「視点と語り」, fofr "Use the active voice", fofr "Address the reader as 'you'" は**却下** (日本語不適)

## 2.5 一文の長さ (fofr 新規)

- **一文 60 字を目安、100 字を上限** とする｡英文の "15-20 words" を日本語に翻案した閾値｡
- **読点 3 個以上** または **節 3 個以上** を含む文は分割を検討する｡
- 句点で文を切る方が、読点で接続するより読みやすい｡長くなるなら段落を分けるか、文を二つに切る｡
- 例外: コードブロックやコマンド例、引用文はこの限りではない｡
- 文書の種類別の調整: ADR/Design Doc/書籍の章では厳しく適用｡文体優先の文書 (Slack/Flow/議事録) では会話的な長さも許容する｡

典拠: 結城浩『数学文章作法 推敲編』(60字)、Microsoft 日本語スタイルガイド (50字)、fofr "Keep sentences short — about 15 to 20 words"

## 2.6 平易語の選択 (fofr 新規 — 方針のみ)

事務文体・漢語重ね・カタカナ官庁用語を避け、和語または平易な漢語で書く｡

**方針** (具体的な置換リストは作らない):

- 「実施する」「対応する」「展開する」のような便利動詞を、文脈に応じた具体的な動詞 (「やる」「直す」「配る」など) に置き換えるか検討する｡
- 「現時点」「鑑みる」「斟酌」のような事務文体の語は、個人文体が許容する文書 (Slack/Flow) では平易語に｡fofr 強化規範が優先される文書 (ADR/Design Doc) でも、漢語重ね (「対応実施」「検討対応」など) を避ける｡

**置換リストを作らない理由**: 日本語の置換は文脈依存が強い｡「実施する→やる」は技術ブログでは適切でも、ADR では砕けすぎる｡個人文体の指紋とも干渉する｡画一化は害が大きいので、方針だけ示す｡

典拠: 文化庁「公用文作成の考え方」(平易性)、fofr "Use everyday words"

## 2.7 数字・日付・記号の表記 (fofr 新規)

- **アラビア数字は半角**｡漢数字は慣用語 (一つ・二人・三日月・四季・五十音など) のみ｡
- 桁区切りはコンマ (1,000,000)｡
- **% や ℃ は半角**｡
- **日付は西暦 4 桁・半角アラビア**｡「2026年6月30日」または「2026-06-30」(ISO 8601)｡和暦は公的引用以外で使わない｡
- **時刻は 24 時制・半角**｡「10:00」「23:59」｡「午前10時」「午後11時59分」のような表記は使わない｡
- **範囲は波ダッシュ「〜」(U+301C)**｡「10:00〜11:30」｡全角チルダ `～` (U+FF5E) は使わない｡
  書き手が「から」や「-」を選ぶのは自由だが、校正で「〜」を書き換えない｡
- **コロンは常に半角 `:`**｡全角コロン「：」は使わない｡
- **& は使わず** 「と」「および」「・」のいずれか｡固有名詞の一部 (例: Procter & Gamble) は除く｡

典拠: 共同通信社『記者ハンドブック』、文化庁「公用文作成の考え方」、fofr "Numbers, Dates and times"

ただしコロンの半角は上の典拠に反する｡『記者ハンドブック』と「公用文作成の考え方」は
いずれも全角コロンを採る｡本スキルは「句読点・括弧・記号 (, : ; ! ?) は半角、中点「・」と
波ダッシュ「〜」は全角」を規則として置く｡典拠ではなく本スキルの決めごとで、
半角句読点は同梱の textlint config が強制する｡

## 2.8 箇条書きと番号付きリスト (fofr 新規)

- **箇条書きを導入する文末は半角コロン `:` で受ける**｡「次の通り:」「対象は次の三つ:」のように｡
- 項目の **文体を揃える** (体言止めまたは「〜する」で統一)｡名詞・動詞・文を混ぜない｡
- 項目末は **句点なし**｡ただし項目自身が完全な複文の場合のみ句点を付ける｡
- 項目の途中に「および」「または」を入れない｡
- 用語とその定義を並べるときは、区切り線ではなく **半角コロン** で「**用語**: 説明」と書く (2.1 と整合)｡
- **番号付きリストは順序のある手順だけ** に使う｡並列の列挙には番号なしのリストを使う｡
- 番号付きリストの各 Step は完全な文で書き、句点で終わる｡

典拠: fofr "Bullet points / Numbered steps"

## 2.9 略号・英文略号 (fofr 新規)

- 日本語の **「等」「例えば」「すなわち」「など」「つまり」** は標準語彙｡禁止しない｡
- **英文略号 (e.g., i.e., etc., N.B., cf., vs.)** は日本語文中で使わない｡和訳して書く｡
...
- 専門略語 (URL、HTTP、JSON、ADR、PR など) は使ってよい｡初出時に必要なら展開を添える｡

典拠: fofr "Do not use Latin abbreviations"

## 2.10 リンクテキスト (fofr 新規)

- リンクテキストは **行き先が分かる句** にする｡
- 「こちら」「click here」「詳細」「上記リンク」のような **内容のないリンクテキスト** を使わない｡
- 悪い例: 「詳細は[こちら](url)」
- 良い例: 「[日本語技術文書の文章規範](url) を参照」

典拠: fofr "Descriptive link text", WCAG (アクセシビリティ)
...
- **感嘆符・ALL CAPS は使わない** (fofr 由来で強化)｡fofr "Do not use exclamation marks / ALL CAPS" と整合｡
- 帰結の列挙によって事故や危険を煽らない｡
...
- **FAQ 形式で代用しない** (fofr 由来で追加)｡読者ニーズを Phase 1.1 で特定した上で、本文で直接答える｡

典拠: k16shikano「演出の抑制」, fofr "Do not use exclamation marks / ALL CAPS / FAQs"
...
### 2.12.1 preset で機械検出するもの

太字強調 (`**注意**` 等)・コロン継続 (述語で終わる文にコロンを付けてリストやコードブロックを続ける形｡コロンの幅は問わない)・hype 表現 (「革命的」「次世代」「パラダイムシフト」)・冗長表現 (「まず最初に」「することができる」) は [[textlint-check]] が `@textlint-ja/preset-ai-writing` および `preset-ja-technical-writing` で検出する｡生成時に意識して避ける必要はあるが、規範の詳細はそちらに委ねる｡

コロン継続を見る `no-ai-colon-continuation` は形態素解析で述語終わりと体言終わりを区別する｡
「〜を挙げる:」は指摘され、「マージの規則:」のような体言のラベルは通る｡箇条書きやコードブロックを
導入するときは、文で書くなら句点で終え、コロンで受けるなら体言のラベルにする｡

### 2.12.2 生成時に意識して避けるもの (preset 範囲外)
...
- **空虚な強調**: 「非常に」「極めて」「大いに」 (程度だけを盛って量を示さない)
- **空虚な動詞**: 「掘り下げる」「深掘りする」「言語化する」 (何をどう書いたかを示さず終わる)、「触れる」「言及する」 (一段落で済ませるだけ)
- **接続の型**: 「〜において」「〜という側面から」「〜の観点から」 (新情報なし)
- **弱い緩和**: 「〜と言えるだろう」 (根拠なく主張を弱める場合だけ｡推量・仮定・読者の疑念・作中人物の認識なら残す)
...
- **見出しの長さは全角 30〜40 字を目安** (fofr "Headings under 65 characters" の日本語翻案)｡
- **見出し末尾に句点・?・! を付けない** (fofr "No full stop, dash, slash or question mark" の翻案)｡疑問形は「？」を付けずに「〜か」「〜は何か」で終える｡

典拠: k16shikano「見出しの付け方」, fofr "Headings"

## 2.15 読み手の負荷の管理

読者の記憶と注意は有限の資源として扱う｡(Phase 1.6「書かないことの決定」と対をなす｡Phase 1.6 は方針、本節は執筆中の実装)
...
文書の種類の判定 (Phase 1.2) が **書籍の章 / blog 長文 / 長文ノート** のときは、執筆に入る前に `references/cognitive-rhythm.md` を Read する｡本規範は「読者を最後まで読ませるための緊張とリズム」を扱い、Phase 2 の他節 (論証・段落・整形) とは別レイヤーである｡

文書の種類の判定が **ADR / Design Doc / Stock 文書** のときは、同 references の「1. 開幕の緊張」と「4. 状況 vs 文書テスト」だけを弱適用する｡他の 8 節 (認知モード交替・文のビート・段落密度波など) は Design Doc のフォーマルさと衝突するので適用しない｡

文書の種類の判定が **Slack / Flow 文書 / 議事録** のときは適用しない｡短文で緊張設計は不要である｡

典拠: k16shikano「認知リズム」<https://gist.github.com/k16shikano/eb2929f13ed19c97188393d297be8432>
